...
<div class="ui segment right aligned">
  <a href="{{ route('plugin.adaptbb.topics.add', [ 'forum_slug' => $forum->slug ]) }}"
     class="ui right labeled icon button primary large">
    Create Topic
    <i class="plus icon"></i>
  </a>
</div>
...
@if(!$topics->count())
  <div class="ui text container center aligned very padded">
    <p>No topics made. <a href="{{ route('plugin.adaptbb.topics.add', [ 'forum_slug' => $forum->slug ]) }}">Get the
        party started!</a>
  </div>
  @else
    <!-- TABLET AND DESKTOP -->
  <table class="ui red stackable very padded large topics table tablet only computer only">
    ...
    @foreach($topics as $topic)
      <tr>
        <td>
          <a
            href="{{ route('plugin.adaptbb.topics.view', [ 'forum_slug' => $forum->slug, 'topic_slug' => $topic->slug ]) }}">
            <strong>{{ $topic->name }}</strong>
          </a><br/>

          <small>By: <a
              href="{{ route('users.profile.view', [ 'username' => $topic->user->username ]) }}">{{ $topic->user->username }}</a> @ {{ Core::getDateLong($topic->created_at) }}
          </small>
        </td>
        <td>
            ...
            <div class="ui feed pull-right">
              <div class="event">
                ...
                <div class="content">
                  ...
                  <div class="summary">
                    {{ $last_reply->user->getName() }} posted <a
                      href="{{ $last_reply->getUrl() }}">{{ $last_reply->name }}</a>
                  </div>
                </div>
              </div>
            </div>
