      ...
      <div class="ui cms dropdown">
        ...
        <div class="menu">
          <div class="{{ !Request::get('category_id') ? 'active' : '' }} item">
            <a
              href="{{ route('admin.posts.index', [ 'category_id' => '', 'status' => Request::get('status') ]) }}">Any</a>
          </div>

          @foreach($categories as $category)
            <div class="{{ Request::get('category_id') == $category->id ? 'active' : '' }} item">
              <a
                href="{{ route('admin.posts.index', [ 'category_id' => $category->id, 'status' => Request::get('status') ]) }}">{{ $category->name }}</a>
            </div>
          @endforeach
        </div>
      </div>
      ...
      <div class="ui cms dropdown">
        ...
        <div class="menu">
          <div class="{{ Request::get('status', 1) === '' ? 'active' : '' }} item">
            <a
              href="{{ route('admin.posts.index', [ 'category_id' => Request::get('category_id'), 'status' => '' ]) }}">Any</a>
          </div>
