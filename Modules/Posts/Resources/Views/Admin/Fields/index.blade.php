      ...
      <div class="ui cms dropdown">
        ...
        <div class="menu">
          <div class="{{ !Request::get('category_id') ? 'active' : '' }} item">
            <a
              href="{{ route('admin.fields.index', [ 'category_id' => '', 'field_type' => Request::get('field_type') ]) }}">Any</a>
          </div>

          @foreach($categories as $category)
            <div class="{{ Request::get('category_id') == $category->id ? 'active' : '' }} item">
              <a
                href="{{ route('admin.fields.index', [ 'category_id' => $category->id, 'field_type' => Request::get('field_type') ]) }}">{{ $category->name }}</a>
            </div>
          @endforeach
        </div>
      </div>
      ...
      <div class="ui cms dropdown">
        ...
        <div class="menu">
          <div class="{{ !Request::get('field_type') ? 'active' : '' }} item">
            <a
              href="{{ route('admin.fields.index', [ 'category_id' => Request::get('category_id'), 'field_type' => '' ]) }}">Any</a>
          </div>
          @foreach($field_types as $key => $type)
            <div class="{{ Request::get('field_type') == $key ? 'active' : '' }} item">
              <a
                href="{{ route('admin.fields.index', [ 'category_id' => Request::get('category_id'), 'field_type' => $key ]) }}">{{ $type }}</a>
            </div>
          @endforeach
        </div>
      </div>
