    ...
    <table class="ui stackable compact celled definition table themes">
      ...
      @foreach($items as $item)
        <tr>
          ...
          <td>
            @if(empty($item->id))
              <a href="{{ route('admin.themes.activate', [ 'slug' => $item->slug ]) }}"
                 class="ui right labeled icon button green">Activate <i class="warning icon"></i></a>
            @else
              @if(!$item->status)
                <a href="{{ route('admin.themes.status', [ 'id' => $item->id ]) }}"
                   class="ui right labeled icon button green">Make Active <i class="check icon"></i></a>
              @endif

              <a href="{{ route('admin.themes.edit_templates', [ 'id' => $item->id ]) }}"
                 class="ui right labeled icon button primary">Files <i class="pencil icon"></i></a>
            @endif
          </td>
        </tr>
      @endforeach
