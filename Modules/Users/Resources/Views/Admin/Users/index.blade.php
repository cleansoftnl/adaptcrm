  ...
  <div class="ui stackable center aligned grid">
    ...
    <div class="ui right floated right aligned ten wide column index-nav"><span>Show me users by</span>
      ...
      <div class="ui cms dropdown">
        ...
        <div class="menu">
          <div class="{{ !Request::get('role_id') ? 'active' : '' }} item"><a
              href="{{ route('admin.users.index', [ 'role_id' => '', 'status' => Request::get('status', 1) ]) }}">Any</a>
          </div>
      ...
      <div class="ui cms dropdown">
        ...
        <div class="menu">
          <div class="{{ Request::get('status', 1) === '' ? 'active' : '' }} item"><a
              href="{{ route('admin.users.index', [ 'role_id' => Request::get('role_id'), 'status' => '' ]) }}">Any</a>
          </div>

          <div class="{{ Request::get('status', 1) === 1 ? 'active' : '' }} item"><a
              href="{{ route('admin.users.index', [ 'role_id' => Request::get('role_id'), 'status' => 1 ]) }}">Active</a>
          </div>

          <div class="{{ Request::get('status', 1) === '0' ? 'active' : '' }} item"><a
              href="{{ route('admin.users.index', [ 'role_id' => Request::get('role_id'), 'status' => 0 ]) }}">Pending</a>
          </div>
        </div>
      </div>
    ...
    <table class="ui stackable compact celled definition table users">
      ...
      @foreach($items as $item)
        <tr>
          ...
          <td><a
              href="{{ route('admin.users.edit', [ 'id' => $item->id ]) }}"><strong>{{ $item->username }}</strong></a>
            <a href="{{ route('admin.users.login_as', [ 'id' => $item->id ]) }}"
               class="ui right labeled icon right floated mini green button">
              Login As
          ...
          <td>
            <div class="ui labeled button" tabindex="0"
                 data-tooltip="Assigned Roles: {{ implode(', ', $item->roles->pluck('name')->toArray()) }}">
