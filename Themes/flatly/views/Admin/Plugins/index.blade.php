...
@section('content')
  ...
  <div class="ui eight wide column pull-right index-nav">
    ...
    <a href="{{ route('admin.updates.index', [ 'module_type' => 'plugins' ]) }}"
  ...
  <table class="ui stackable compact celled table plugins">
    ...
    @foreach($items as $item)
      <tr>
        ...
        <td>
          @if(!$item['enabled'])
            <a href="{{ route('admin.plugins.install', [ 'slug' => ucfirst($item['slug']) ]) }}"
               class="ui right labeled icon button primary">
              Install <i class="plus icon"></i>
            </a>
          @elseif(!in_array($item['name'], $core_modules))
            <a href="{{ route('admin.plugins.uninstall', [ 'slug' => ucfirst($item['slug']) ]) }}"
