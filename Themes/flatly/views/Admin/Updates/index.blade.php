...
@section('content')
  ...
  <div class="ui red padded segment">
    ...
    <div>
      <a href="{{ route('admin.updates.upgrade', [ 'type' => 'normal' ]) }}"
         class="ui right labeled icon green big button {{ !Cache::has('cms_latest_version') ? 'disabled' : '' }}">
  ...
  <div class="ui text container">
    ...
    @if(!Core::getAddonUpdates('plugins'))
      ...
      <a href="{{ route('admin.updates.browse', [ 'module_type' => 'plugins']) }}"
         class="ui right labeled icon floated primary button pull-right">
    ...
    @else
      <table class="ui form compact celled definition table plugins update">
          ...
          <tr>
            <td class="collapsing">
              <div class="ui fitted slider checkbox">
                <input type="checkbox" class="plugin-id" v-model="module_ids[{{ $module['id'] }}]" checked>
                <label></label>
              </div>
            </td>
            <td>{{ $module['name'] }}</td>
        ...
        <tr>
          <th></th>
          <th colspan="4">
            <a href="{{ route('admin.updates.browse', [ 'module_type' => 'plugin']) }}"
               class="ui right labeled icon floated small primary button pull-right">
              <i class="shopping basket icon"></i> Find More Plugins
            </a>
      ...
      <table class="ui form compact celled definition table themes update">
        ...
        @foreach(Core::getAddonUpdates('themes') as $module)
          <tr>
            <td class="collapsing">
              <div class="ui fitted slider checkbox">
                <input type="checkbox" class="theme-id" v-model="module_ids[{{ $module['id'] }}]" checked>
                <label></label>
              </div>
            </td>
            <td>{{ $module['name'] }}</td>
        ...
        <tr>
          <th></th>
          <th colspan="4">
            <a href="{{ route('admin.updates.browse', [ 'module_type' => 'theme']) }}"
               class="ui right labeled icon floated small green button pull-right">
              <i class="shopping basket icon"></i> Find More Themes
            </a>
            <a href="" @click.prevent="update()" class="ui small button">
              Update
