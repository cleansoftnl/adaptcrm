...
<head>
  ...
  <link rel="stylesheet" type="text/css" href="/themes/portfolio/assets/css/main.compiled.min.css">
      ...
      <div class="three wide column">
        ...
        <div class="ui inverted link list">
          @foreach(Core::getData('categories', 'all', [], [ 'ord', 'asc' ], 5) as $category)
            <a href="{{ route('categories.view', [ 'slug' => $category->slug ]) }}"
               class="{{ Request::url() == route('categories.view', [ 'slug' => $category->slug ]) ? 'active' : '' }} item">
              {{ $category->name }}
            </a>
          @endforeach
        </div>
      ...
      <div class="three wide column">
        ...
        <div class="ui inverted link list">
          @foreach(Core::getData('pages', 'all', [], [], 5) as $page)
            ...
            @else
              <a href="{{ route('pages.view', [ 'slug' => $page->slug ]) }}"
                 class="{{ Request::url() == route('pages.view', [ 'slug' => $page->slug ]) ? 'active' : '' }} item">
                {{ $page->name }}
              </a>
            @endif
          @endforeach
        ...
        <div class="ui inverted link list">
          @foreach(Core::getData('albums', 'all', [], [], 5) as $album)
            <a href="{{ route('albums.view', [ 'slug' => $album->slug ]) }}"
               class="{{ Request::url() == route('albums.view', [ 'slug' => $album->slug ]) ? 'active' : '' }} item">
              {{ $album->name }}
            </a>
          @endforeach
        </div>
