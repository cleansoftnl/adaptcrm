<h1 class="text-color-black">{{ $category->name }}</h1>
    ...
    @else
      @foreach($posts['posts_with_data'] as $post)
        <div class="item">
          ...
          <div class="content">
            <a href="{{ route('posts.view', [ 'slug' => $post['post']->slug ]) }}"
               class="header">{{ $post['post']->name }}</a>
            <div class="meta">
              ...
              @if(!empty($post['tags']))
                @foreach($post['tags'] as $tag)
                  <div class="ui label"><a
                      href="{{ route('tags.view', [ 'slug' => $tag->slug ]) }}">{{ $tag->name }}</a></div>
                @endforeach
              @endif
            </div>
          </div>
